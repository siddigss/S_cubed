...
batch_size = 1
im_size = 64
out_size = 64 #output size of the CNN result.
...
for i in range(num_ims):
    # interpolation to increase the number of vertices of ground truths from 4 to L (Why?)
    poly = np.zeros([5, 2])
    ...
    curve = []
    for c in range(4):
        number_of_vertices = L//4 + min(1,max(L%4 - c,0))
        for j in range(number_of_vertices):
            curve.append(poly[c,:] + (j/number_of_vertices)*(poly[c+1,:]-poly[c,:]))
    ground_thruths[:,:,i] = np.array(curve)
    #loading images
    images[:,:,:,i] = np.float32(imageio.imread(data_path+'building_'+str(push+i)+'.png'))/255
