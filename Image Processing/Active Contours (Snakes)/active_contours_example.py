...
from active_contours import active_contours
...
Number_of_edges = 60                                                                                                 #of the polygon

center_x = 215                                                                                                               #Setting the initial curve
center_y = 400
radius = 30
...
pixel_factor = 1                                                                                                              #Effectiveness of the forces
fig = plt.figure()
while True:                                                                                                                      #Keep pressing SPACE BAR to see the evolution
    if plt.waitforbuttonpress():
        snake = active_contours(Energy, snake, 10, 1*pixel_factor, 1*pixel_factor)
        Energy_img2 = Image.open(Image_name)
        draw = ImageDraw.Draw(Energy_img2)
        snake_tuples = []
        for i in range(Number_of_edges):
            snake_tuples.append((snake[i,1], snake[i,0]))
        snake_tuples.append((snake[0,1], snake[0,0]))
        draw.line(snake_tuples,  fill=(0,255,0), width=3)
